...
allowed-tools: Bash(git ls-files:*), Bash(git log:*), Bash(git shortlog:*), Bash(git diff:*), Bash(git rev-parse:*), Bash(wc:*), Read, Grep, Glob, Task, AskUserQuestion, Write, Edit
---
...
Then one `AskUserQuestion` confirming the classification and the scope. If `$ARGUMENTS` names a path,
restrict the campaign to it. This is the only question before the chunk map.
...
one revision round, then write **or skip**. Fan out the pre-reads one agent per chunk across the
...
**5 — Compress.** Run `/audit-intent-layer` over the paths just written, **before** the commit. Reusing
...
questions parked, tasks recorded. Then sample the last 30 commits and count how many touched a
directory that now has a node without touching that node — that is the rate at which the commit hook
will now speak. Under about 1 in 3 is healthy. Above it, merge nodes upward before landing.
...
A boundary with no stated rule gets no directory.
