---
name: capture-intent-layer
description: Establishing an intent layer on a repo that has none — chunking a codebase at semantic boundaries, interviewing the subject-matter expert leaf-first, and rolling children's nodes up into parents without re-reading their code. Use when a repo has no CLAUDE.md hierarchy, when a single root CLAUDE.md should be several nodes, or when laying down nodes for a project before its code exists. Triggers on "set up the intent layer", "bootstrap CLAUDE.md", "capture the intent layer", "we have no CLAUDE.md files", "split the root CLAUDE.md", "interview me about this codebase".
---
...
**When the LCA is the repo root**, work down this table before adding to the root node. The root
always loads, which makes it the most expensive real estate in the repo.

| If the fact | Goes to | Because |
|---|---|---|
| Genuinely holds everywhere | Root node | That is what a root node is for. |
| Is shaped like a file type rather than a location | `.claude/rules/*.md` with `paths:` | Globs by pattern instead of by location. Confirm it actually fires. |
| Is a repeatable procedure | Skill | On demand, so it costs nothing to sessions that don't need it. |
| Must hold under pressure | Hook | A node is advisory. |
